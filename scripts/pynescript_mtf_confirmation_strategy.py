...
# Docs: strategy-authoring-handbook.html#mtf-patterns
# Docs: function-cookbook.html#request-wrong-right
import numpy as np
import pandas as pd

from source.gui.scripts_editor.pynescript_runtime import strategy, input, ta, request, build_mapped_trade_frame
...
def build_signal_frame(df: pd.DataFrame, params: dict | None = None) -> pd.DataFrame:
    ...
    # Higher-timeframe confirmation is part of signal computation, not trade mapping.
...
def build_trade_frame(signal_df: pd.DataFrame, params: dict | None = None, styles: dict | None = None) -> pd.DataFrame:
    # Mapping stays thin after all signals and mapped fields are prepared above.
