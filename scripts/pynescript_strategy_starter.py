...
# Docs: strategy-authoring-handbook.html#mapped-fields
...
def build_signal_frame(df: pd.DataFrame, params: dict | None = None) -> pd.DataFrame:
    ...
    # mapped_* fields belong in build_signal_frame because they are strategy intent.
    ...
    frame["mapped_size_pct"] = float(p.get("size_pct", 0.0) or 0.0)
    return frame
