# @name: pynescript_utbot_strategy_full
import numpy as np
import pandas as pd
...
from source.gui.scripts_editor.pynescript_runtime import strategy, input


strategy("Pyne UTBOT Strategy Full", overlay=True, process_orders_on_close=True)
...
trade_qty = input.float(0.0, title="Fixed Qty", key="trade_qty")
...
def build_signal_frame(df: pd.DataFrame, params: dict | None = None) -> pd.DataFrame:
    ...
    signal_frame["visual_show"] = signal_frame.get("mapped_arrow_direction", pd.Series("", index=signal_frame.index)).astype(str).ne("")
    signal_frame["visual_x"] = pd.to_numeric(signal_frame.get("index", pd.Series(dtype=float)), errors="coerce").fillna(0.0)
    signal_frame["visual_y"] = pd.to_numeric(signal_frame.get("mapped_arrow_price", pd.Series(dtype=float)), errors="coerce").ffill().fillna(0.0)
    signal_frame["visual_direction"] = signal_frame.get("mapped_arrow_direction", pd.Series(dtype=object)).astype(str)
    signal_frame["visual_color"] = signal_frame.get("mapped_arrow_color", pd.Series(dtype=object)).astype(str)
    signal_frame["visual_size"] = pd.to_numeric(signal_frame.get("mapped_arrow_size", pd.Series(dtype=float)), errors="coerce").fillna(15).astype(int)
    return signal_frame
...
def build_visuals(frame: pd.DataFrame, params: dict | None = None, ctx=None):
    ...
    return ctx.atk.color_arrow(
        key="utbot_strategy_full_arrows",
