...
from source.gui.scripts_editor.pynescript_runtime import indicator, input, linefill, plot, ta
...
# plot(...) points to dataframe columns that will be created later in build_indicator_frame.
...
# linefill.new(...) links the named line visuals, so it uses plot keys rather than dataframe column names.
...
def build_indicator_frame(df: pd.DataFrame, params: dict | None = None) -> pd.DataFrame:
    ...
    try:
        # ta.bbands returns upper, basis, lower in one grouped call.
    ...
    # These output column names must match the plot source names declared above.
