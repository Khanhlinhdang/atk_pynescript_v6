...
def build_signal_frame(df: pd.DataFrame, params: dict | None = None) -> pd.DataFrame:
    ...
    frame["mapped_entry_order_type"] = "STOP_LIMIT"
    frame["mapped_entry_price"] = frame["close"]
    frame["mapped_entry_trigger_price"] = np.where(
        frame["mapped_entry_side"] == "BUY",
        trigger_high,
        np.where(frame["mapped_entry_side"] == "SELL", trigger_low, 0.0),
    )
    frame["mapped_entry_limit_price"] = np.where(
        frame["mapped_entry_side"] == "BUY",
        buy_limit,
        np.where(frame["mapped_entry_side"] == "SELL", sell_limit, 0.0),
    )
    frame["mapped_quantity"] = float(p["trade_qty"])
    frame["mapped_size_pct"] = 0.0
    frame["mapped_time_in_force"] = np.where(frame["mapped_entry_side"] != "", "GTD", "GTC")
    frame["mapped_expiration"] = np.where(
        frame["mapped_entry_side"] != "",
        pd.to_numeric(frame["time"], errors="coerce").fillna(0).astype("int64") // 1000 + 3600,
        0,
    )
    frame["mapped_tag"] = np.where(frame["mapped_entry_side"] != "", "STOP_LIMIT_ENTRY", "")
    return frame
