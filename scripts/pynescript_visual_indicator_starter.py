...
# Docs: visual-authoring-handbook.html#render-only
# Docs: function-cookbook.html#build-visuals-wrong-right
import pandas as pd

from source.gui.scripts_editor.pynescript_runtime import indicator, input, ta, plot, plotshape, hline
...
def build_indicator_frame(df: pd.DataFrame, params: dict | None = None) -> pd.DataFrame:
    ...
    if not frame.empty:
        ...
        # attrs carries static object config, while build_visuals reads the current frame slice.
...
def build_visuals(frame: pd.DataFrame, params: dict | None = None, ctx=None):
    if frame is None or frame.empty or ctx is None:
        return None
    # build_visuals stays render-only and consumes the prepared payload.
