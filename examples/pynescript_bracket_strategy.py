...
def build_signal_frame(df: pd.DataFrame, params: dict | None = None) -> pd.DataFrame:
    ...
    frame["sl"] = np.where(
        ...
        frame["entry_price"] - (frame["atr"] * float(p["sl_atr"])),
        np.where(frame["entry_side"] == "SELL", frame["entry_price"] + (frame["atr"] * float(p["sl_atr"])), 0.0),
    )
    frame["tp"] = np.where(
        frame["entry_side"] == "BUY",
        frame["entry_price"] + (frame["atr"] * float(p["tp_atr"])),
        np.where(frame["entry_side"] == "SELL", frame["entry_price"] - (frame["atr"] * float(p["tp_atr"])), 0.0),
    )
    return frame
