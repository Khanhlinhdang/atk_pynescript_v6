...
# Docs: strategy-authoring-handbook.html#trailing-fields
...
def build_signal_frame(df: pd.DataFrame, params: dict | None = None) -> pd.DataFrame:
    ...
    # mapped_trail_offset belongs in the signal frame because trailing is part of strategy intent.
    frame["mapped_trail_offset"] = frame["atr"] * float(p["trail_mult"])
    return frame


def build_trade_frame(signal_df: pd.DataFrame, params: dict | None = None, styles: dict | None = None) -> pd.DataFrame:
    # Keep the trade frame mapping-only.
