# @name: pynescript_pyramiding_strategy
...
strategy("Pyramiding Strategy", overlay=True, pyramiding=1, process_orders_on_close=True)
fast_period = input.int(3, title="Fast", key="fast_period")
slow_period = input.int(8, title="Slow", key="slow_period")
...
def build_signal_frame(df: pd.DataFrame, params: dict | None = None) -> pd.DataFrame:
    ...
    frame["entry_side"] = np.where(
        frame["buy_signal"] | frame["add_signal"],
        "BUY",
        np.where(frame["sell_signal"], "SELL", ""),
    )
    frame["entry_price"] = frame["open"]
    frame["quantity"] = float(p.get("trade_qty", 0.0) or 0.0)
    frame["size_pct"] = 0.0
    frame["tag"] = np.where(frame["buy_signal"], "L1", np.where(frame["add_signal"], "L2", ""))
    return frame
